...
Straight_Length = 0 ;
  ...
  {
    if(R_Huan_Time_Counter>30||Lost_Left[30]||Lost_Left[32])       //左边线丢失，或时间过长
    {
      R_Huan_Time_Counter = 0;
  ...
  if(!R_Huan_Exit_Flag&&R_Huan_Flag&&R_Huan_Time_Counter<40)  //判断为环岛，且时间小于20，为环岛入口 
  {
    R_Huan_Entry_Flag = 1; 
  }
  else
