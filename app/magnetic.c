...
void magnet_deal()
{
  DirectionErr[0] = (float)(Inductance[4]-Inductance[5]+Inductance[0]*zhang-Inductance[3]*zhang)/(Inductance[4]+Inductance[5]+Inductance[0]*zhang+Inductance[3]*zhang);//左-右水平电感的差比和作为偏差
	...
	if(Flag_Round==0)
  {
    if(DirectionErr[0]<0.15&&DirectionErr[0]>-0.15)
    {
      servo_Kp=kpyu;
      servo_Kd=kdyu;
      DirectionOut=DirectionErr[0]*(Direction_P-800)+DirectionErr_dot[0]*(Direction_D-1000);//这一部分由于水平电感位置不同要根据一定规律设置
    }
    else
    {
      servo_Kp=kpzhi;
